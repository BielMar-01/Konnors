<?php
    ...
    $chartGroup = get_sub_field('chartGenerator');
    $chartID = $chartGroup['chartID'];
?>
<div class="wpContent--chart">
    <section id="<?= $chartID ?>" class="container"></section>
    <?php
        $graph = $chartGroup['chart'];
    ...
    <script>
        <?php 
            $colors = array();
            foreach($chartGroup['colors'] as $color){
        ...
        <?php echo "chartOptions_$sectionIndex"; ?>['colors'] = <?php echo json_encode($colors); ?>;

        Highcharts.chart('<?= $chartID ?>', <?php echo "chartOptions_$sectionIndex"; ?>);
    </script>
</div>